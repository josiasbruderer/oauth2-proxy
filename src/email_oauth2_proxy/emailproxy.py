...
LOG_FILE_PATH = '%s/%s.log' % (os.path.dirname(os.path.realpath(WORKING_DIRECTORY)), APP_SHORT_NAME)

CONFIG_FILE_PATH = '%s/%s.config' % (os.path.dirname(os.path.realpath(WORKING_DIRECTORY)), APP_SHORT_NAME)
...
EXITING = False  # used to c


class UpdatePath():

    def __init__(self, wd):
        global CONFIG_FILE_PATH, WORKING_DIRECTORY, LOG_FILE_PATH

        WORKING_DIRECTORY = wd
        CONFIG_FILE_PATH = '%s/%s.config' % (WORKING_DIRECTORY, APP_SHORT_NAME)
        LOG_FILE_PATH = '%s/%s.log' % (WORKING_DIRECTORY, APP_SHORT_NAME)
    ...
    def _log(level_method, level, *args):
        ...
        if len(message) > 2048 and (sys.platform not in ['win32', 'darwin'] or Log._MACOS_USE_SYSLOG):
            truncation_message = ' [ NOTE: message over syslog length limit truncated to 2048 characters; run `%s' \
                                 ' --debug` in a terminal to see the full output ] ' % os.path.basename(WORKING_DIRECTORY)
